...
export default function LearnMorePage() {
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-slate-900 overflow-x-hidden">
      {/* Navigation */}
      <nav className="p-4 md:p-6 flex justify-between items-center max-w-6xl mx-auto border-b border-black/5 dark:border-slate-700">
      ...
      <main className="max-w-4xl mx-auto px-4 md:px-6 py-12 md:py-16">
        ...
        >
          <motion.h1 
            variants={fadeIn}
            className="text-4xl md:text-6xl font-extrabold text-black dark:text-slate-100 mb-6 tracking-tight px-2"
          >
            Work with your <span className="text-blue-600">Biology</span>, not against it.
          </motion.h1>
          <motion.p 
            variants={fadeIn}
            className="text-lg md:text-xl text-black/70 dark:text-white/70 leading-relaxed max-w-2xl mx-auto"
          >
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-center mb-20 md:mb-24">
          ...
          >
            <h2 className="text-2xl md:text-3xl font-bold text-black dark:text-slate-100 mb-6">The Energy-First Approach</h2>
            <p className="text-base md:text-lg text-black/90 dark:text-slate-300 mb-6 leading-relaxed">
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={index} className={`flex gap-4 p-4 ${item.bg} dark:bg-slate-800 rounded-2xl border ${item.border} dark:border-slate-700`}>
                  <item.icon className={`${item.color} w-6 h-6 shrink-0`} />
                  <p className="text-black dark:text-slate-300 text-sm md:text-base font-medium">
                    <strong className="text-black dark:text-slate-100">{item.title}</strong> {item.desc}
                  </p>
                </div>
              ))}
            </div>
          ...
          <motion.div 
            ...
            className="bg-slate-900 dark:bg-slate-800 dark:border dark:border-blue-500/30 dark:shadow-blue-500/10 rounded-[2rem] md:rounded-[2.5rem] p-8 md:p-10 text-white shadow-2xl mt-8 md:mt-0"
          >
  ...
  <h2 className="text-3xl font-bold text-black dark:text-slate-100 mb-12 text-center">The FocusFlow Engine</h2>
  <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
    {[
      ...
      <div key={i} className="relative p-8 bg-white dark:bg-slate-800 border border-black/5 dark:border-slate-700 rounded-[2rem] shadow-sm hover:shadow-lg transition-shadow">
        <span className="text-4xl font-black text-black/5 absolute top-4 right-6">{item.step}</span>
        <h3 className="text-xl font-bold text-black dark:text-slate-100 mb-3">{item.title}</h3>
        <p className="text-black/90 dark:text-slate-300 text-lg leading-relaxed">{item.desc}</p>
      </div>
    ))}
  </div>
</section>
        ...
        >
          <div className="inline-flex items-center gap-2 bg-gray-200 dark:bg-slate-700 px-4 py-2 rounded-full mb-6">
            <ShieldCheck className="w-6 h-6 text-blue-600" />
            <span className="text-sm md:text-lg font-bold text-black dark:text-slate-100">Privacy Focused</span>
          </div>
          <h2 className="text-2xl md:text-3xl font-bold text-black dark:text-slate-100 mb-6 px-4">Your Data Stays Yours</h2>
          <p className="md:text-lg text-black dark:text-slate-300 max-w-2xl mx-auto px-4">
        ...
        <motion.section 
          whileHover={{ y: -5 }}
          className="bg-slate-50 dark:bg-slate-800 border border-black/5 dark:border-slate-700 rounded-[2rem] md:rounded-[3rem] p-8 md:p-12 text-center shadow-inner"
        >
          <h2 className="text-2xl md:text-3xl font-bold text-black dark:text-slate-100 mb-4">Ready to reach your peak?</h2>
          <p className="text-black/90 dark:text-slate-300 mb-8 max-w-md mx-auto text-sm md:text-base px-2">
